"""Terminal and JSON output for extracted Hirist jobs."""
...
def print_jobs_by_experience_city_company(
    ...
    print(f"\nTotal jobs (exp/city/company): {total}")
...
def write_jobs_json(
    ...
    *,
    ...
    json_path = Path(path) if path is not None else Path(__file__).resolve().parents[1] / "output" / "hirist" / "jobs.json"
