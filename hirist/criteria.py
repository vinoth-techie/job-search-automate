"""Local filters: ignored titles, keyword match, freshness."""
...
# Drop jobs that list any of these skills (comma-separated tags).
_SKILL_IGNORE = frozenset({"shopify"})
...
def skills_are_ignored(skills: str) -> bool:
    if not skills:
        return False
    for part in str(skills).split(","):
        if part.strip().casefold() in _SKILL_IGNORE:
            return True
    return False


def job_is_ignored(job: dict[str, Any]) -> bool:
    return title_is_ignored(str(job.get("title") or "")) or skills_are_ignored(
...
def filter_jobs(
    ...
    *,
    ...
    """Drop ignored titles/skills; optionally require keyword match and freshness."""
