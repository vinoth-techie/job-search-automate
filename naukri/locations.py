...
# Canonical city -> substrings that appear in Naukri location labels
...
def group_by_city(
    ...
    *,
    ...
    """Group jobs under target cities (flat list per city)."""
...
def group_by_city_and_company(
    ...
    *,
    ...
    """
    Group jobs as city -> company -> jobs[].

    Companies are sorted alphabetically within each city.
    Multi-city listings appear under each matching city.
    """
...
def group_by_experience_city_company(
    ...
    *,
    ...
    """
    Group as experience -> city -> company -> jobs[].

    Top keys are Naukri search experience filters, e.g. "3", "4".
    """
    keys = experience_keys or sorted(jobs_by_experience.keys(), key=lambda k: int(k) if k.isdigit() else k)
    return {
        key: group_by_city_and_company(jobs_by_experience.get(key) or [], cities=cities)
        for key in keys
    }
